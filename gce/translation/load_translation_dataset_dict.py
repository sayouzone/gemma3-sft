...
# 데이터 준비 (딕셔너리의 리스트 형태)
my_data = [
    {"english": "I heard something creeping in the dark.", "korean": "어둠 속에서 무언가가 기어오는 소리가 들렸다."}
]

# Dataset.from_dict() 또는 Dataset.from_list() 사용
# 딕셔너리의 키가 열(column) 이름이 됨
raw_dataset = Dataset.from_dict({"english": [d["english"] for d in my_data], "korean": [d["korean"] for d in my_data]})
#raw_dataset = Dataset.from_list(my_data)
