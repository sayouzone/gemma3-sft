...
class Gemma3TPUTrainer:
    ...
    def __init__(self, model_name="gemma-2b"):
        ...
        if "gemma-3" in model_name:
            self.model_id = f"google/{model_name}-it"
        else:
    ...
    def process_dataset(self, dataset_name="Abirate/english_quotes"):
        data = load_dataset(dataset_name)

        def preprocess_function(data):
            # Create prompt, completion, and combined text columns
            prompts = [f"Generate a quote:\n\n" for _ in data["quote"]]
            completions = [f"{quote}{self.tokenizer.eos_token}" for quote in data["quote"]]
    ...
    def config_fsdp(self):
        fsdp_training_args = fsdp_v2.get_fsdp_training_args(self.model)
        ...
        return fsdp_training_args_copy, fsdp_config_value
    ...
    def config_lora(self):
        ...
        print(f"Reloading model and tokenizer: {self.model_id}")
        ...
        model = AutoModelForCausalLM.from_pretrained(self.model_id, config=config, torch_dtype=torch.bfloat16)
    ...
    def train_model(self, data, fsdp_training_args_copy, fsdp_config_value):
        ...
        sft_config = SFTConfig(
            ...
            num_train_epochs=1,
            max_steps=-50,
            output_dir=f"./output/{self.model_name}-finetuned",
            optim="adafactor",
            logging_steps=50,
...
if __name__ == "__main__":
    ...
    gemma_trainer = Gemma3TPUTrainer(model_name=model_name)
    quote_dataset = gemma_trainer.process_dataset(dataset_name=dataset_name)
    fsdp_training_args_copy, fsdp_config_value = gemma_trainer.config_fsdp()
    gemma_trainer.config_lora()
    gemma_trainer.train_model(quote_dataset, fsdp_training_args_copy, fsdp_config_value)