...
class StorageAPI:
    ...
    def download_gcs_folder_concurrently(self, source_folder, destination_folder):
        ...
        for blob in blobs:
            ...
            destination_path = os.path.join(destination_folder, os.path.basename(blob.name))
            #print("blob", blob)
            #print("blob", blob.name, blob.size, blob.chunk_size)

            if blob.size > 32 * 1024 * 1024:
                print(f"'{blob.name}' 파일을 청크 단위로 병렬 다운로드 시작")
                ...
                try:
                    ...
                    blob.download_to_filename(destination_path)
                    print("Downloaded {}.".format(destination_path))
                except Exception as e:
                    print(f"'{blob.name}' 파일 다운로드 중 오류 발생: {e}")
    ...
    def download_chunks_concurrently(
        ...
        print("Downloaded {} Concurrently.".format(filename))
